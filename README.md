...
## Features
- **Human-Friendly URLs:** Replaces complex, parameter-heavy phpBB links with clean, descriptive URLs (e.g., `your-topic-title-t10.html`).
...
A: This means your server is not yet configured to handle the rewritten URLs. You must go to the extension's settings in the ACP, under the "Server Configuration" tab, and copy the required rules into your Apache `.htaccess` file or your NGINX server block.

**Q: Does this extension work on NGINX servers?**
A: Yes! The ACP module provides the exact rewrite rules you need for both Apache and NGINX based on your selected rewrite mode.
...
A: Simple mode uses minimal text (e.g., `forum123.html`), keeping URLs as short as possible. Advanced mode includes the forum or topic titles within the URL (e.g., `my-topic-title-t123.html`), which provides more context and readability.
...
If you decide to uninstall this extension, it is **crucial** to implement fallback redirection rules to prevent 404 errors. The URLs previously indexed by search engines (e.g., `topic-title-t10.html`) need to smoothly redirect back to the standard phpBB format (`viewtopic.php?t=10`) using 301 Permanent Redirects.
...
# 1. Redirect post links (e.g., slug-t12-p34.html or post-p34.html)
RewriteRule ^.*-t([0-9]+)-p([0-9]+)\.html$ ./viewtopic.php?t=$1&p=$2 [QSA,R=301,L]
RewriteRule ^.*-p([0-9]+)\.html$ ./viewtopic.php?p=$1 [QSA,R=301,L]

# 2. Redirect topic links (e.g., slug-t123.html or topic-t123.html)
RewriteRule ^.*-t([0-9]+)\.html$ ./viewtopic.php?t=$1 [QSA,R=301,L]

# 3. Redirect forum links (e.g., slug-f45.html or forum-f45.html)
RewriteRule ^.*-f([0-9]+)\.html$ ./viewforum.php?f=$1 [QSA,R=301,L]
```
...
# 1. Redirect post links (e.g., slug-t12-p34.html or post-p34.html)
rewrite ^/.*-t([0-9]+)-p([0-9]+)\.html$ /viewtopic.php?t=$1&p=$2 permanent;
rewrite ^/.*-p([0-9]+)\.html$ /viewtopic.php?p=$1 permanent;

# 2. Redirect topic links (e.g., slug-t123.html or topic-t123.html)
rewrite ^/.*-t([0-9]+)\.html$ /viewtopic.php?t=$1 permanent;

# 3. Redirect forum links (e.g., slug-f45.html or forum-f45.html)
rewrite ^/.*-f([0-9]+)\.html$ /viewforum.php?f=$1 permanent;
```
